...
extract_split_info = function(tree, split_benchmark = NULL) {
  ...
  rows = lapply(unlist(tree, recursive = FALSE), function(node) {
    ...
    row = data.frame(
      ...
      int_imp = if (is.null(node$importance)) NA else round(node$importance$int_imp, 2),
      int_imp_remaining = if (is.null(node$importance) || is.null(node$importance$int_imp_remaining))
        NA else round(node$importance$int_imp_remaining, 2),
