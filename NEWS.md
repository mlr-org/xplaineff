...
- default_predict_fun() now uses direct regression prediction paths for native `ranger`, native and mlr3 `rpart`, and native and mlr3 `xgboost` models when no custom `predict_fun` is supplied (no issue).
